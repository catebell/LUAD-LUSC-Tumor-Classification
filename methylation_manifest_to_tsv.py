...
def see_example_values():
    ...
    file_mapping_df = pd.read_csv(f'{config.FILES}/{config.tumor}/clinical/file_case_mapping.tsv', sep='\t', dtype=str)

    case_id = 'fd5c44ef-ea50-4fba-9e8d-e371cf34ebdb' # for example

    df_meth = pd.read_csv(f"{config.FILES}/{config.tumor}/methylation/{file_mapping_df[
