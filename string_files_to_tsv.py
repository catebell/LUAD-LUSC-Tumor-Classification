...
def create_protein_links():
    """
    Run to re-create the 9606.protein.links.v12.0.txt file with combined_score in a [0-1] interval.
    Output will be saved in 'output_filepath' as a tsv.
    """

    output_filepath = "downloaded_files/9606.protein.links.v12.0.tsv"
    ...
    protein_links_df.to_csv(output_filepath, sep="\t", index=False)


def create_protein_aliases_gene():
    """
    Run to re-create the 9606.protein.aliases.gene.txt file with protein_id, gene_name and gene_id mapping and excluding
    data not useful. Output will be saved in 'output_filepath' as a tsv.
    Takes a lot of time.
    """
